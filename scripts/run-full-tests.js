...
class TestLogger {
  ...
  log(level, category, message, metadata = {}) {
    ...
    console.log(`${color}[${level.toUpperCase()}]${colors.reset} ${colors.cyan}[${category}]${colors.reset} ${message}`);
  ...
  generateSummary() {
    const totalTests = Object.values(testState.results).reduce((acc, result) => 
      acc + result.passed + result.failed + result.skipped, 0);
    
    const totalPassed = Object.values(testState.results).reduce((acc, result) => 
      acc + result.passed, 0);
    
    const totalFailed = Object.values(testState.results).reduce((acc, result) => 
      acc + result.failed, 0);

    const totalDuration = Object.values(testState.results).reduce((acc, result) => 
      acc + result.duration, 0);
...
class CommandExecutor {
  ...
  async runHealthCheckFallback(endpoint, startTime) {
    try {
      ...
      if (healthCheck.success) {
        this.logger.log('success', 'HEALTH', `Health check passou (fallback): ${endpoint}`, healthCheck);
      } else {
        this.logger.log('error', 'HEALTH', `Health check falhou (fallback): ${endpoint}`, healthCheck);
      }
      ...
      testState.healthChecks.push(healthCheck);
      this.logger.log('warn', 'HEALTH', `Health check não pôde ser executado: ${endpoint}`, healthCheck);
      return healthCheck;
    }
  }
}
...
class TestRunner {
  ...
  async checkServices() {
    ...
    this.logger.log('info', 'SYSTEM', `Verificação de serviços concluída: ${activeServices}/${totalServices} ativos`, {
      services: serviceResults
    });

    // Definir comportamento baseado em serviços ativos
    if (activeServices === 0) {
      this.logger.log('warn', 'SYSTEM', 'Nenhum serviço ativo detectado. Alguns testes podem falhar.');
    }
  ...
  async runUnitTests() {
    ...
    try {
      ...
      this.logger.log('error', 'UNIT_TESTS', 'Erro ao executar testes da API', { error: error.message });
    ...
    try {
      ...
      this.logger.log('error', 'UNIT_TESTS', 'Erro ao executar testes do Frontend', { error: error.message });
    ...
    this.logger.log('info', 'UNIT_TESTS', `Testes unitários finalizados: ${successCount}/${totalCount} componentes passaram`, {
      results: testResults,
  ...
  async runIntegrationTests() {
    ...
    try {
      ...
      await this.executor.runCommand('npm run test -- --testNamePattern="integration|Integration" --ci', {
        cwd: path.join(rootDir, 'apps', 'api'),
        testType: 'integration'
      });

    } catch (error) {
      this.logger.log('error', 'INTEGRATION_TESTS', 'Falha nos testes de integração', { error: error.message });
    }
  }
  ...
  async runPerformanceTests() {
    ...
    try {
      ...
      await this.executor.runCommand('npm run test -- --testNamePattern="performance|Performance|timing|load" --ci', {
        cwd: path.join(rootDir, 'apps', 'api'),
      ...
      this.logger.log('error', 'PERFORMANCE_TESTS', 'Falha nos testes de performance', { error: error.message });
    }
  }
  ...
  async runSecurityTests() {
    ...
    try {
      // Testes de segurança
      await this.executor.runCommand('npm run test -- --testNamePattern="security|Security|auth|Auth|validation" --ci', {
        cwd: path.join(rootDir, 'apps', 'api'),
        testType: 'security'
      });

    } catch (error) {
      this.logger.log('error', 'SECURITY_TESTS', 'Falha nos testes de segurança', { error: error.message });
    }
  }
  ...
  async runPerformanceBenchmarks() {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        this.logger.log('error', 'BENCHMARK', `Erro no benchmark: ${endpoint}`, { error: error.message });
      }
    }
  }
  ...
  displaySummary(summary) {
    ...
    console.log(`  • Duração Total: ${colors.bright}${(summary.totalDuration / 1000).toFixed(2)}s${colors.reset}`);
    ...
    Object.entries(testState.results).forEach(([category, result]) => {
      ...
      if (total > 0) {
        console.log(`  ${category.toUpperCase()}: ${colors.green}${result.passed}✓${colors.reset} ${colors.red}${result.failed}✗${colors.reset} ${colors.yellow}${result.skipped}⊘${colors.reset} (${(result.duration / 1000).toFixed(2)}s)`);
      }
    });
    ...
    console.log(`  • Performance: ${colors.magenta}${testState.logs.performance.length}${colors.reset}`);
...
async function main() {
  ...
  try {
    ...
    if (!skipRestart) {
      console.log(`${colors.bright}${colors.blue}🔄 Reinicializando sistema antes dos testes...${colors.reset}`);

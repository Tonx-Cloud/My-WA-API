...
class ProjectValidator {
    ...
    for (const [app, vars] of Object.entries(requiredEnvVars)) {
      try {
        ...
        const missing = vars.filter(
          (varName) => !envObj[varName] && !process.env[varName],
        );

        if (missing.length > 0) {
          this.errors.push(
            `Variáveis faltantes em ${app}: ${missing.join(", ")}`,
          );
        } else {
        ...
        this.errors.push(
          `Erro ao validar variáveis de ${app}: ${error.message}`,
        );
      }
    }
  }
  ...
  async validateRoutes() {
    ...
    try {
      ...
      try {
        ...
        this.warnings.push(
          `Não foi possível verificar rotas da API: ${error.message}`,
        );
      }

      // Verificar rotas do Web (Next.js App Router)
      const webRoutesPath = path.join(
        process.cwd(),
        "apps",
        "web",
        "src",
  ...
  async validateContainers() {
    ...
    try {
      ...
      expectedServices.forEach((service) => {
        ...
        if (container) {
          if (container.State === "running") {
            ...
            this.errors.push(
              `❌ Container ${service} não está rodando: ${container.State}`,
            );
          }
        } else {
      ...
      this.warnings.push(
        `Não foi possível verificar containers: ${error.message}`,
      );
    }
  }
  ...
  async validateWebSocketConnection() {
    ...
    try {
      ...
      this.warnings.push(
        "⚠️ Teste de WebSocket requer implementação adicional",
      );
    } catch (error) {
  ...
  async validateReactComponents() {
    ...
    try {
      ...
      if (
        ...
        this.successes.push(
          "✅ Dashboard component exporta default corretamente",
        );
      } else {
  ...
  async validateDatabase() {
    ...
    try {
      ...
      this.warnings.push(
        `Não foi possível verificar PostgreSQL: ${error.message}`,
      );
    }
  }
  ...
  async validateLogs() {
    ...
    try {
      ...
      if (logFiles.length > 0) {
        this.successes.push(
          `✅ Sistema de logs ativo (${logFiles.length} arquivos)`,
        );
      } else {
  ...
  async validateSessions() {
    ...
    try {
      ...
      try {
        ...
        this.successes.push(
          `✅ Diretório de sessões encontrado (${sessions.length} sessões)`,
        );
      } catch {
        this.warnings.push(
          "⚠️ Diretório de sessões não encontrado - será criado automaticamente",
        );
      }
    } catch (error) {
  ...
  generateReport() {
    ...
    const score = Math.round(
      (this.successes.length / (this.successes.length + this.errors.length)) *
        100,
    );
    this.log(

...
class PostCommitValidator {
  ...
  async runAllValidations() {
    ...
    for (const validation of validations) {
      ...
      try {
        ...
        console.error(
          `❌ Erro na validação ${validation.label}:`,
          error.message,
        );
  ...
  async validatePerformance() {
    ...
    try {
      const apiDistSize = await this.getDirectorySize(
  ...
  printFinalSummary() {
    ...
    if (failedCategories > 0) {
      ...
      console.log(
        "2. Execute novamente: node scripts/post-commit-validation.js",
      );
  ...
  validator
    ...
    .then((results) => {
      ...
      fs.writeFile(resultFile, JSON.stringify(results, null, 2))
        ...
        .catch((err) =>
          console.log(`⚠️ Não foi possível salvar resultados: ${err.message}`),
        );

      ...
      if (result.instances) {
        ...
        const instances = result.instances.map((instance: any) => {
          ...
          if (
            ...
            instance.whatsapp_status === "ready"
          ) {
            status = "connected";
          } else if (
            ["qr_ready", "authenticated", "loading"].includes(instance.status)
          ) {
            status = "connecting";
          }
    ...
    return NextResponse.json(
      {
        ...
        message:
          "Unable to fetch instances. Please check if the API service is running.",
      },
      { status: 503 },
    );
  } catch (error) {
      ...
      if (response.ok) {
        ...
        if (result.instance || result.message) {
          const newInstance = {
            ...
            name:
              result.instance?.name || body.name || `Instância ${instanceId}`,
            status: "connecting",
      ...
      return NextResponse.json(
        {
          error: "Backend API unavailable",
          message:
            "Unable to create instance. Please check if the API service is running.",
          details:

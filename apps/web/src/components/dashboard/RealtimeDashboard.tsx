...
export const RealtimeDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('instance_status_updated', (data: { instanceId: string; status: string; phone?: string }) => {
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {recentMessages.length > 0 
                  ? Math.round((recentMessages.filter(m => m.status === 'delivered').length / recentMessages.length) * 100)
                  : 0}%
              </p>
            </div>
          </div>
                ...
                <div key={instance.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div className={`w-3 h-3 rounded-full ${
                      ...
                      instance.status === 'qr_pending' ? 'bg-blue-500' : 'bg-red-500'
                    }`}></div>
                    <div>

          ...
          return (
            <div key={metric.name} className="bg-white rounded-lg shadow-lg border p-6">
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-lg ${getTrendBgColor()}`}>{getTrendIcon()}</div>
              </div>
              ...
              <div className="mt-4">
                <div className="flex items-center">
                  <span className={`text-sm font-medium ${getTrendTextColor()}`}>
                    {getTrendArrow()}
                    {Math.abs(((metric.current - metric.previous) / metric.previous) * 100).toFixed(
                      1
                    )}
                    %
                  </span>
          ...
          {formatChartData.slice(-20).map(item => {
            ...
            return (
              <div key={`${item.time}-chart`} className="flex-1 flex flex-col items-center">
                ...
                <span className="text-xs text-gray-500 mt-1 truncate">{item.time}</span>
              </div>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {formatChartData
                .reduce((acc, item) => acc + item.messagesSent + item.messagesReceived, 0)
                .toLocaleString()}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {formatChartData.length > 0
                ? Math.round(
                    formatChartData.reduce((acc, item) => acc + item.responseTime, 0) /
                      formatChartData.length
                  )
                : 0}{' '}
              ms
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">
              {formatChartData.length > 0
                ? (
                    formatChartData.reduce((acc, item) => acc + item.errorRate, 0) /
                    formatChartData.length
                  ).toFixed(1)
                : 0}
              %

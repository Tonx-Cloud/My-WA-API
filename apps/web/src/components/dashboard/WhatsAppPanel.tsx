...
export const WhatsAppPanel: React.FC<WhatsAppPanelProps> = ({ instances: propInstances }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('contact_status_update', (data: { contactId: string; status: 'online' | 'offline' | 'typing' }) => {
  ...
  const loadInstances = async () => {
    try {
      const response = await fetch(`${process.env['NEXT_PUBLIC_API_URL'] || 'http://localhost:3000'}/api/instances`)
      if (response.ok) {
  ...
  const loadContacts = async () => {
    try {
      const response = await fetch(`${process.env['NEXT_PUBLIC_API_URL'] || 'http://localhost:3000'}/api/contacts?instanceId=${selectedInstance}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const loadMessages = async () => {
    ...
    try {
      const response = await fetch(`${process.env['NEXT_PUBLIC_API_URL'] || 'http://localhost:3000'}/api/messages?instanceId=${selectedInstance}&contactId=${selectedContact.id}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(`${process.env['NEXT_PUBLIC_API_URL'] || 'http://localhost:3000'}/api/messages/send`, {
        method: 'POST',
        headers: {
  ...
  const getInstanceStatusColor = (status: string): string => {
    switch (status) {
      ...
      case 'qr_pending': return 'text-blue-600'
        ...
        <div className="bg-gray-50 p-4 border-b border-gray-200">
          ...
          {selectedInstance && (
            <div className="mt-2 text-xs">
              ...
              <span className={`font-medium ${getInstanceStatusColor(instances.find(i => i.id === selectedInstance)?.status || '')}`}>
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">
                  {selectedContact.status === 'online' ? 'online' : 
                   selectedContact.status === 'typing' ? 'digitando...' : 
                   'offline'}
                </p>
              </div>
              <div className="flex items-center space-x-2">

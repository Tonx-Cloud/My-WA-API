...
describe("API Services - Enhanced Test Suite - Phase 4 Fixed", () => {
  beforeEach(() => {
    ...
    mockWhatsAppService.sanitizeInput.mockImplementation((input) => ({
      sanitized:
        typeof input === "string" ? input.replace(/<script>/g, "") : input,
    ...
    mockWhatsAppService.trackOperationTiming.mockImplementation(
      (operationId) => ({
  ...
  describe("Data Validation", () => {
    it("should validate instance creation data", () => {
      ...
      expect(mockWhatsAppService.validateInstanceData).toHaveBeenCalledWith(
        instanceData,
      );
    });
    ...
    it("should validate message data", () => {
      ...
      expect(mockWhatsAppService.validateMessageData).toHaveBeenCalledWith(
        messageData,
      );
    });
    ...
    it("should validate webhook data", () => {
      ...
      expect(mockWhatsAppService.validateWebhookData).toHaveBeenCalledWith(
        webhookData,
      );
    });
  });
  ...
  describe("Performance and Reliability", () => {
    it("should track operation timing", () => {
      ...
      expect(mockWhatsAppService.trackOperationTiming).toHaveBeenCalledWith(
        operationId,
      );
    });
    ...
    it("should handle concurrent operations", async () => {
      ...
      expect(mockWhatsAppService.handleConcurrentOps).toHaveBeenCalledWith(
        operations,
      );
    });
  ...
  describe("Security and Input Validation", () => {
    it("should sanitize input data", () => {
      const maliciousInput =
        'Clean text <script>alert("xss")</script> more text';
      const result = mockWhatsAppService.sanitizeInput(maliciousInput);

      expect(result.sanitized).not.toContain("<script>");
      expect(result.wasSanitized).toBe(true);
      expect(mockWhatsAppService.sanitizeInput).toHaveBeenCalledWith(
        maliciousInput,
      );
    });
  ...
  describe("Error Handling Scenarios", () => {
    it("should handle network timeouts", async () => {
      ...
      expect(mockWhatsAppService.handleNetworkTimeout).toHaveBeenCalledWith(
        operation,
      );
    });
    ...
    it("should handle invalid credentials", async () => {
      ...
      const result =
        await mockWhatsAppService.handleInvalidCredentials(credentials);

      expect(result.error).toBe("Invalid credentials");
      expect(result.code).toBe("AUTH_FAILED");
      expect(result.retryable).toBe(false);
      expect(mockWhatsAppService.handleInvalidCredentials).toHaveBeenCalledWith(
        credentials,
      );
    });
    ...
    it("should handle service unavailable", async () => {
      ...
      const result =
        await mockWhatsAppService.handleServiceUnavailable(request);

      expect(result.error).toBe("Service temporarily unavailable");
      expect(result.code).toBe("SERVICE_UNAVAILABLE");
      expect(result.retryAfter).toBeGreaterThan(0);
      expect(mockWhatsAppService.handleServiceUnavailable).toHaveBeenCalledWith(
        request,
      );
    });
  });

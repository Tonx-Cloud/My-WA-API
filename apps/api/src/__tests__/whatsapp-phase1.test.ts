...
describe('WhatsApp Service Mock Tests - Phase 1', () => {
  ...
  describe('Instance Management', () => {
    ...
    test('should get instance status correctly', () => {
      ...
      expect(mockWhatsAppService.getInstanceStatus).toHaveBeenCalledWith('test-instance-id');
    });
    ...
    test('should delete instance successfully', async () => {
      ...
      expect(deleteResult).toHaveProperty('message', 'Instance deleted successfully');
      expect(mockWhatsAppService.deleteInstance).toHaveBeenCalledWith(instanceId);
    });
  });
  ...
  describe('Connection Management', () => {
    test('should connect instance successfully', async () => {
      ...
      const connectResult = await mockWhatsAppService.connectInstance(instanceId);
      ...
      expect(mockWhatsAppService.connectInstance).toHaveBeenCalledWith(instanceId);
    });

    test('should disconnect instance successfully', async () => {
      const instanceId = 'test-instance-123';

      const disconnectResult = await mockWhatsAppService.disconnectInstance(instanceId);
  ...
  describe('Message Sending', () => {
    test('should send message successfully', async () => {
      ...
      const messageResult = await mockWhatsAppService.sendMessage(instanceId, to, message);
      ...
      expect(mockWhatsAppService.sendMessage).toHaveBeenCalledWith(instanceId, to, message);
    });
    ...
    test('should send bulk messages successfully', async () => {
      ...
      const bulkResult = await mockWhatsAppService.sendBulkMessage(instanceId, recipients, message);
  ...
  describe('Session Management', () => {
    test('should save session successfully', async () => {
      ...
      const saveResult = await mockWhatsAppService.saveSession(instanceId, sessionData);

      expect(saveResult).toHaveProperty('success', true);
      expect(saveResult).toHaveProperty('message', 'Session saved successfully');
      expect(mockWhatsAppService.saveSession).toHaveBeenCalledWith(instanceId, sessionData);
    });
    ...
    test('should clear session successfully', async () => {
      ...
      expect(clearResult).toHaveProperty('message', 'Session cleared successfully');
  ...
  describe('Webhook Management', () => {
    test('should set webhook successfully', async () => {
      ...
      const webhookResult = await mockWhatsAppService.setWebhook(instanceId, webhookUrl, events);
      ...
      expect(mockWhatsAppService.setWebhook).toHaveBeenCalledWith(instanceId, webhookUrl, events);
    });
  });
  ...
  describe('Instance Information', () => {
    test('should get instance info successfully', async () => {
      ...
      expect(mockWhatsAppService.getInstanceInfo).toHaveBeenCalledWith(instanceId);
    });
  });
});

...
describe('WhatsApp-Web.js Mocks - Phase 6', () => {
  ...
  describe('Message Operations', () => {
    ...
    test('deve enviar mensagem com sucesso', async () => {
      ...
      const message = await client.sendMessage('mock-contact-1@c.us', 'Hello test message');
  ...
  describe('Search and Utilities', () => {
    ...
    test('deve obter foto de perfil', async () => {
      const profilePicUrl = await client.getProfilePicUrl('mock-contact-1@c.us');
  ...
  describe('Error Handling', () => {
    test('deve falhar operações quando cliente não está pronto', async () => {
      ...
      await expect(unreadyClient.getContacts()).rejects.toThrow('Client not ready');
    });
  });

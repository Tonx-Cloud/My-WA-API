...
export function initializePassport() {
  ...
  try {
    ...
    if (existingProviderUser) {
      logger.info(`Login OAuth Google realizado via provider_id: ${existingProviderUser.email}`);
...
passport.deserializeUser(async (id: number, done) => {
  try {
    ...
    logger.warn('⚠️  Google OAuth não configurado. Defina GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET no .env');
  }
}

...
async function testXAI() {
  try {
    ...
    const chatResponse = await xaiClient.chatCompletion([
      {
        role: 'system',
        content: 'Você é um assistente de atendimento ao cliente especializado em e-commerce.',
      },
      {
        role: 'user',
    ...
    console.log(`✅ Chat com contexto: ${chatResponse.choices[0]?.message?.content}\n`);

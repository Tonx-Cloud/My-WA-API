...
describe("BackupService - Restore & Cleanup (Item 3)", () => {
  ...
  describe("Advanced Cleanup Logic", () => {
    test("deve implementar lógica de retenção por tipo de backup", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const incremental = await backupService.createBackup(
          sources,
          "incremental",
          { test: `old-inc-${i}` },
        );
      ...
      const incrementalBackups = beforeCleanup.filter(
    ...
    test("deve limpar arquivos temporários órfãos", async () => {
      ...
      const orphanFiles = [
        "temp_backup_123.tmp",
        "temp_restore_456.tmp",
        "partial_upload_789.tmp",
      ];
    ...
    test("deve implementar cleanup em lote com limite de taxa", async () => {
      ...
      for (
        let i = 0;
        i < Math.min(backupIds.length, maxDeletionsPerBatch);
        i++
      ) {
        try {

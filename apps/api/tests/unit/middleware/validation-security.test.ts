...
describe('Validation & Security Middlewares (Item 4)', () => {
  ...
  describe('Input Validation Middleware', () => {
    ...
    test('deve validar parâmetros de URL', async () => {
      ...
      app.get('/test/:id/:action', validateParams(paramSchema), (req: Request, res: Response) => {
    ...
    test('deve validar headers HTTP', async () => {
      ...
      app.post('/test', validateHeaders(headerSchema), (req: Request, res: Response) => {
        res.json({ success: true });
      });
  ...
  describe('Security Middleware', () => {
    test('deve extrair contexto de segurança da requisição', async () => {
      ...
      await request(app).get('/test').set('User-Agent', 'Mozilla/5.0 Test Browser').expect(200);
    ...
    test('deve detectar tentativas de SQL injection', async () => {
      ...
      await request(app).post('/test').send({ query: "' OR '1'='1" }).expect(400);
    });
    ...
    test('deve detectar tentativas de XSS', async () => {
      ...
      await request(app).post('/test').send({ link: 'javascript:alert("xss")' }).expect(400);
    });

    test('deve validar tokens JWT', async () => {
      const mockJWTValidation = (token: string): { valid: boolean; payload?: any } => {
  ...
  describe('Input Sanitization', () => {
    test('deve escapar caracteres HTML perigosos', async () => {
      ...
      const response = await request(app).post('/test').send(dangerousInput).expect(200);

      expect(response.body.data.message).toBe(
        '&lt;script&gt;alert(&quot;xss&quot;)&lt;/script&gt;'
      );
      expect(response.body.data.description).toBe('Hello &amp; &quot;goodbye&quot;');
    ...
    test('deve remover caracteres de controle perigosos', async () => {
      ...
      const response = await request(app).post('/test').send(inputWithControlChars).expect(200);
  ...
  describe('Advanced Security Features', () => {
    ...
    test('deve detectar user agents suspeitos', async () => {
      const suspiciousPatterns = [/bot/i, /crawler/i, /spider/i, /scanner/i, /sqlmap/i, /nikto/i];
      ...
      // User agent suspeito
      await request(app).get('/test').set('User-Agent', 'sqlmap/1.0').expect(403);
    });

...
async function runExamples() {
  ...
  try {
    const xaiClient = getXAIClient();
    const text =
      "Adorei o produto! Chegou rapidinho e a qualidade é excepcional.";
  ...
  try {
    ...
    const analysis = await whatsappService.analyzeMessage(
      message,
      "João Silva",
    );
    ...
    console.log(
      "✅ Intenção de compra:",
      JSON.stringify(purchaseIntent, null, 2),
    );
  ...
  try {
    ...
    const chatResponse = await xaiClient.chatCompletion([
      ...
      },
      {
        role: "user",
        content:
          "Estou procurando um celular bom e barato para minha mãe de 70 anos.",
      },
    ]);
    console.log(
      `✅ Assistente de vendas: ${chatResponse.choices[0]?.message?.content}\n`,
    );
  } catch (error) {
    console.log(`❌ Erro: ${error.message}\n`);
  }
